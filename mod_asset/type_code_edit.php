...
$stmt = $db->prepare("SELECT `atc_seq_no`, `atc_type_desc`
    FROM `asset_type_code` WHERE `atc_seq_no` = ? LIMIT 1");
$stmt->bind_param("s", decode($_GET['id']));
$stmt->execute();
$stmt->bind_result($atc_seq_no,$atc_type_desc);
$stmt->fetch();
$stmt->close();
$db->close();
?>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function(){
		...
		for(var i=0; i<len; i++){
                ...
                //$('#n_error').hide();
                $("#form1").attr('action','type_code_edit_save.php');
        ...
        $("#btnclose").click( function(){
             var mainP = parent.document.getElementById('tcewindow');
             mainP.close();
           }
        );
...
<ul class="form-style-1">
     <input type="hidden" id="txtseqno" name="seq_no" value="<?php echo encode($atc_seq_no); ?>"/>
    <li>
   <label for="name">Type Code Description<span class="required">*</span></label>
    <input type="text" id="txtdescription" name="description" value="<?php echo $atc_type_desc; ?>" class="field-long"/>
    </li>
    <li>
