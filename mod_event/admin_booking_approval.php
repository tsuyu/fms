...
//dumper($_POST)
//echo encode("admin_booking_approval_detail");
//echo encode("admin_booking_approval_save");
?>
...
<table class="pure-table" width="100%">
    ...
    <tbody>
        ...
        $sql = "SELECT DATE_FORMAT(`ebm_from_date`,'%d-%m-%Y'),TIME_FORMAT(`ebm_from_date`, '%h:%i%p'),DATE_FORMAT(`ebm_to_date`,'%d-%m-%Y'),TIME_FORMAT(`ebm_to_date`, '%h:%i%p'),`aum_name`, 
                `ebm_purpose`,`ebm_status`,`ebm_seq_no`
        ...
        $stmt->execute();
        $stmt->bind_result($ebm_from_date, $ebm_from_time, $ebm_to_date, $ebm_to_time, $aum_name, $ebm_purpose, $ebm_status, $ebm_seq_no);
        while ($stmt->fetch()) {
            if ($y % 2 == 0) {
            ...
            <tr <?php echo $odd; ?>>
                ...
                <td><button name="approve" class="pure-button pure-button-primary_g" onclick="ns.approve(<?php echo $ebm_seq_no; ?>); return false" type="button" id="approve">Detail</button></td>
            </tr>
            <?php
            $y++;
        }
...
<script type="text/javascript">
    ...
    namespace = {
        ...
        approve: function (id) {
            location.href = "home.php?mod=u2&app=34h5d4a4s56454o4l4t5e4n4b42414u5l4k4m4m4i5p5h5k5a416a4q5t5&id="+id;
        }
    };
    window.ns = namespace;
    ...
    $(document).ready(function () {
        $("#search").click(function () {
            $("#mode").val("search");
            $("#form2").attr('action', 'home.php?mod=u2&app=34h5d4a4s56454o4l4t5e4n4b42414u5l4k4m4m4i5p5');
