...
$stmt = $db->prepare("SELECT `cm_seq_no`, `cm_campus_desc`
    FROM `campus_main` WHERE `cm_seq_no` = ? LIMIT 1");
$stmt->bind_param("s", decode($_GET['id']));
$stmt->execute();
$stmt->bind_result($cm_seq_no,$cm_campus_desc);
$stmt->fetch();
$stmt->close();
$db->close();
?>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function(){
		...
		for(var i=0; i<len; i++){
                ...
                //$('#n_error').hide();
                $("#form1").attr('action','campus_code_edit_save.php');
        ...
        $("#btnclose").click( function(){
             var mainP = parent.document.getElementById('ccewindow');
             mainP.close();
           }
        );
...
<ul class="form-style-1">
     <input type="hidden" id="txtseqno" name="seq_no" value="<?php echo encode($cm_seq_no); ?>"/>
    <li>
   <label for="name">Campus Code Description<span class="required">*</span></label>
    <input type="text" id="txtdescription" name="description" value="<?php echo $cm_campus_desc; ?>" class="field-long"/>
    </li>
    <li>
