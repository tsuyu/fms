...
$stmt = $db->prepare("SELECT `arm_seq_no`,`arm_area_desc`
    FROM `area_main` WHERE `arm_seq_no` = ? LIMIT 1");
$stmt->bind_param("s", decode($_GET['id']));
$stmt->execute();
$stmt->bind_result($arm_seq_no,$arm_area_desc);
$stmt->fetch();
$stmt->close();
$db->close();
?>
...
<script type="text/javascript" language="javascript">
    $(document).ready(function(){
		...
		for(var i=0; i<len; i++){
                ...
                //$('#n_error').hide();
                $("#form1").attr('action','area_code_edit_save.php');
...
<ul class="form-style-1">
     <input type="hidden" id="txtseqno" name="seq_no" value="<?php echo encode($arm_seq_no); ?>"/>
    <li>
   <label for="name">Area Code Description<span class="required">*</span></label>
    <input type="text" id="txtdescription" name="description" value="<?php echo $arm_area_desc; ?>" class="field-long"/>
    </li>
    <li>
