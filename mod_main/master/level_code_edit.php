...
$stmt = $db->prepare("SELECT `blm_seq_no`, `blm_level_desc`
    FROM `building_level_main` WHERE `blm_seq_no` = ? LIMIT 1");
$stmt->bind_param("s", decode($_GET['id']));
$stmt->execute();
$stmt->bind_result($blm_seq_no,$blm_level_desc);
$stmt->fetch();
$stmt->close();
$db->close();
?>
    ...
    $(document).ready(function(){
        $('#btnsubmit').click(function(event) {
            ...
            $('#txtdescription').validator({
                fieldLabel : '- Level Code Description',
                invalidEmpty: true,
		...
		for(var i=0; i<len; i++){
                ...
                //$('#n_error').hide();
                $("#form1").attr('action','level_code_edit_save.php');
        ...
        $("#btnclose").click( function(){
             var mainP = parent.document.getElementById('lcewindow');
             mainP.close();
           }
        );
...
<ul class="form-style-1">
     <input type="hidden" id="txtseqno" name="seq_no" value="<?php echo encode($blm_seq_no); ?>"/>
    <li>
   <label for="name">Level Code Description<span class="required">*</span></label>
    <input type="text" id="txtdescription" name="description" value="<?php echo $blm_level_desc; ?>" class="field-long"/>
    </li>
    <li>
